...
export default function RefundPayments() {
  ...
  useEffect(() => {
    const fetchRefunds = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${API_BASE_URL}/refunds/returned-orders`);
        ...
        console.log("API Response:", res); // Debug the actual response structure

        // Check if response has data (adjust based on your actual API structure)
        const responseData = res.data || res; // Try both common patterns
        ...
        const ordersArray = Array.isArray(responseData) ? responseData : [responseData];

        const transformedData = ordersArray.map(order => ({
          id: `REF-${order._id?.toString().slice(-4).toUpperCase() || '0000'}`,
          orderId: `ORD-${order.orderNumber || order._id?.toString().slice(-4).toUpperCase() || '0000'}`,
          customer: {
            name: order.customer
            ...
            id: `CUST-${order.customer?._id?.toString().slice(-4).toUpperCase() || '0000'}`,
            contact: order.customer?.phone || 'No contact',
            firstName: order.customer?.firstName || order.customer?.name?.split(' ')[0] || 'Unknown'
          },
          amount: order.totalAmount || 0,
          totalAmount: order.totalAmount || 0,
          dateRequested: order.updatedAt ? new Date(order.updatedAt).toISOString().split('T')[0] : 'N/A',
          dateProcessed: order.refundProcessedDate
            ? new Date(order.refundProcessedDate).toISOString().split('T')[0]
            : '',
          ...
          initiatedBy: order.refundInitiatedBy || 'Customer',
          ...(order.status === 'failed' && {
            failureReason: order.refundFailureReason || 'Unknown error'
          })
        }));

        console.log("Transformed Data:", transformedData);
        setRefundPayments(transformedData);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="flex items-center mb-2">
                  <FiUser className="text-gray-400 mr-2" />
                  <span className="font-medium">{selectedRefund.customer.firstName}</span>
                </div>
                <div className="text-sm text-gray-500 mb-1">Customer ID: {selectedRefund.customer.id}</div>
                <div className="text-sm text-gray-500">Contact: {selectedRefund.customer.contact}</div>
              </div>
              ...
              <div key={index} className="bg-white p-4 rounded-lg shadow border border-gray-200 hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="flex items-center mt-1">
                      <span className={`text-sm ${stat.change.startsWith('+') ? 'text-green-500'
                        : stat.change.startsWith('-') ? 'text-red-500'
                          : 'text-gray-500'
                        }`}>
                        {stat.change}
                  ...
                  <div className={`p-2 rounded-full ${stat.change.startsWith('+') ? 'bg-green-100 text-green-600'
                    : stat.change.startsWith('-') ? 'bg-red-100 text-red-600'
                      : 'bg-gray-100 text-gray-600'
                    }`}>
                    <FaRupeeSign className="text-xl" />
        ...
        <div className="fixed z-50 top-0 right-0 left-0 bottom-0 bg-black bg-opacity-30 flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-8 relative">
            ...
            <div className="mb-4 flex flex-col gap-1">
              ...
              <div className="text-sm text-gray-500">Order ID: <span className="font-mono">{selectedRefund.orderId}</span></div>
              <div className="text-sm">Customer: {selectedRefund.customer.name} ({selectedRefund.customer.contact})</div>
              <div className="font-bold text-gray-800 mt-2">Amount: <FaRupeeSign className="inline" />{selectedRefund.totalAmount}</div>
              <div className="text-xs text-gray-500 mt-2">Requested on: {selectedRefund.dateRequested}</div>
            </div>
            <div className="mb-6">
              <label className="block font-medium text-gray-700 mb-2">Select Payment Method:</label>
            ...
            <div className="flex justify-end space-x-2">
              ...
              {paidRefundId === selectedRefund.id ? (
                ...
                <button
                  className={`px-4 py-2 rounded-md bg-green-600 text-white flex items-center space-x-2 font-medium shadow ${payProcessing ? "opacity-70" : "hover:bg-green-700"
                    }`}
                  disabled={payProcessing}
                  onClick={processRefund}
                >
